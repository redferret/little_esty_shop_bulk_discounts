...
RSpec.describe 'The index page for merchants bulk discounts,' do
  ...
  before :each do
    ...
    @discount_1 = FactoryBot.create(:bulk_discount, merchant: @merchant, percentage_discount: 0.6696, quantity_threshold: 10)
  ...
  describe 'discounts list,' do
    it 'shows the discounts with their attributes' do
      within '#discount-list' do
        expect(page).to have_content("1. Discount Offered: 66.96%, Item Threshold: 10")
      end
    end
    
    it 'has a link next to each discount that navigates to that discount show page' do
      expect(page).to have_link("2. Discount Offered: 30.00%, Item Threshold: 15")
      click_link "1. Discount Offered: 66.96%, Item Threshold: 10"
